...
then
	...
	srun -l python /cellar/users/snwright/Git/gwas_pipeline/extract_snp_list.py $snpExtract $extractList

	srun -l plink --bed ${bedfile} --bim ${bimfile} --fam ${famfile} \
	--extract $extractList \
	--freq --out ${outDir}${outName}.freq
	echo "Step\tFrequency"
	srun -l sh /cellar/users/snwright/Git/gwas_pipeline/process_log.sh $3
