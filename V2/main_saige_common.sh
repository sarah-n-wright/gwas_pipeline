...
ids=${outDir}${baseName}.saige.jobIDs
> $ids
...
step1=$(sbatch run_saige.sh $config 1 1)
echo "Step1: " $step1 >> $ids

## Run SAIGE Step 2 ----------------
echo "Submitting Step 2"
step2=$(sbatch --dependency=afterany:$step1 run_saige_step2.sh $config 1)
echo "Step2: " $step2 >> $ids

## Combine stats files -------------------
echo "Submitting Job8a"
job8a=$(sbatch --dependency=afterany:$step2 job8a_post_assoc.sh $config SAIGE imputed)
...
job8b=$(sbatch --dependency=afterany:$job8a job8b_results_plot.sh $config SAIGE combined.IMP.stats)
